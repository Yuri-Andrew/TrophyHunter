	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(wm_full_screen_fragment i) : SV_Target
			{
				...
				fixed4 pixelColor = WM_SAMPLE_FULL_SCREEN_TEXTURE(_MainTex2, i.uv.xy);
				pixelColor.rgb = ComputeAtmosphericScatteringFog(i.uv.xy, _WorldSpaceCameraPos, normalize(i.rayDir), i.forwardLine, fixed4(pixelColor.rgb, 1.0), GetDepth01(i.uv.xy)).rgb;
				return pixelColor;
			}
		...
		Pass
		{
			...
			fixed4 frag(wm_full_screen_fragment i) : SV_Target
			{
				...
				return ComputeAtmosphericLightShafts(i.vertex.xy, _WorldSpaceCameraPos, normalize(i.rayDir), rayLength);
			}
